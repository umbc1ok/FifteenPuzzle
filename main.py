...
def checkMoves(board,lastmove):
    ...
    if (int(index / board.w) != 0) and (lastmove !="D"):
        moves.append("U")
    if int(index / board.w) != 3 and (lastmove !="U"):       # tu jakis error wyskakiwal // jest tutaj hardcodowana szerokosc, trzeba poprawic
        moves.append("D")
...
def dfs(board,depth,maxdepth,lastmove):
    ...
    for move in moves:
        ...
        makeMove(newState,move,findZero(board))
